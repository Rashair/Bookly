...
export default class SearchFlatScreen extends React.Component {
  ...
  setDateFrom(date) {
    this.setState(oldState => ({
      ...
      dateToValid: date ? !!(oldState.dateTo && date < oldState.dateTo) : oldState.dateToValid,
    }));
  }
  ...
  setDateTo(date) {
    this.setState(oldState => ({
      ...
      dateToValid: date ? !!(oldState.dateFrom && oldState.dateFrom < date) : oldState.dateToValid,
    }));
  }
  ...
  createNumberList(n) {
    const table = [];
    for (let i = 1; i <= n; i++) {
      table.push(<Picker.Item label={i.toString()} value={i} key={i} />);
    }
    return table;
  }
  ...
  createChip(i) {
    return (
      <Chip mode="outlined" selected={this.state.beds === i} onPress={() => this.setBeds(i)}>
        {i.toString()}
      </Chip>
    );
  }
  ...
  search() {
    ...
    sendRequest(`${FLATLY_API_URL}/flats`, "GET", /* headers from redux */ body);
  }
  ...
  render() {
    ...
    return (
      <Container style={styles.container}>
        ...
        <HelperText style={styles.helper} type="error" visible={!this.state.dateToValid}>
        ...
        <HelperText style={styles.helper} type="error" visible={!this.state.bedsValid}>
